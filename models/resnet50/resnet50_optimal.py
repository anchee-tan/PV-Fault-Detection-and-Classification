...
def create_resnet50_model(num_classes, unfreeze_percentage=0.5):
    """
    Create ResNet50 with configurable unfreeze percentage and enhanced classifier head.
    
    Args:
        num_classes (int): Number of output classes for the classifier
        unfreeze_percentage (float): Percentage of backbone to unfreeze from end 
                                    (0.25 for last 25%, 0.5 for last 50%, etc.)
    ...
    # ResNet50 has 4 main blocks (conv1, layer1-4) plus fc
    total_blocks = 4  # layer1 through layer4
    ...
    num_unfreeze_blocks = int(total_blocks * unfreeze_percentage)
    if num_unfreeze_blocks == 0 and unfreeze_percentage > 0:
        num_unfreeze_blocks = 1  # Ensure at least one block if percentage > 0
    
    # Calculate which layers to unfreeze
    unfreeze_layers = []
    if unfreeze_percentage > 0:
        # Always unfreeze the classifier (fc)
        unfreeze_layers.append("fc")
        
        # Unfreeze the last N blocks
        start_unfreeze_block = total_blocks - num_unfreeze_blocks + 1  # +1 because layers are 1-indexed
        for i in range(start_unfreeze_block, total_blocks + 1):
    ...
    print(f"ResNet50: Training last {num_unfreeze_blocks} of {total_blocks} main blocks "
          f"({100*num_unfreeze_blocks/total_blocks:.1f}%) plus classifier")
    
    # Enhanced classifier head (same as before)
...
def initialize_model(num_classes, device):
    ...
    model = create_resnet50_model(num_classes, unfreeze_percentage=0.50)
...
def training_model(train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset, num_classes, device, class_names, optimizer_type='AdamW', weighting_method='ens', scheduler_type='cosine', num_epochs=35):
    ...
    print("\nModel Summary")
    model = create_resnet50_model(num_classes, unfreeze_percentage=0.50).to(device)
    try:
    ...
    final_model = create_resnet50_model(num_classes, unfreeze_percentage=0.50).to(device)
