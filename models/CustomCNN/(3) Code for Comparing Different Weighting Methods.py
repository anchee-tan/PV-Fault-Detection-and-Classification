...
def initialize_model(num_classes, device):
    ...
    model_name = "CustomCNN"  # FIXED: Use string instead of class reference
...
def get_class_weights(dataset, num_classes, method='original', beta=None):
    """
    Calculate class weights using different methods - IMPROVED VERSION
    
    Args:
        dataset: Training dataset
        num_classes: Number of classes
        method: Weighting method ('ins', 'isns', 'ens', 'sklearn', 'equal')
    ...
    empty_classes = np.where(samples_per_class == 0)[0]
    if len(empty_classes) > 0:
        print(f"Warning: Found classes with 0 samples: {empty_classes}")
    ...
    if method == 'ins':
        ...
        if beta is None:
            raise ValueError("Beta parameter is required for ENS method")
        
        # Effective Number of Samples: weight_c = 1 / effective_num
        # Handle edge case where beta^n might be very small
        beta_powers = np.power(beta, samples_per_class)
        effective_num = (1.0 - beta_powers) / (1.0 - beta)
        ...
        raise ValueError(f"Unknown method: {method}. Available: ['ins', 'isns', 'ens', 'sklearn', 'equal']")
...
def visualize_class_weights(class_names, class_counts, raw_weights, final_weights, logger=None, method='original'):
...
def setup_optimizer(model, optimizer_type='AdamW', class_weights=None, device='cuda'):
    """
...
def training_model(train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset, num_classes, device, class_names, optimizer_type='AdamW', weighting_method='original', beta=None):
    """Main training pipeline with comprehensive performance tracking - FIXED VERSION"""
    
    # Initialize performance logger and system monitor
    logger = PerformanceLogger(experiment_name=f"CustomCNN_{optimizer_type}_{weighting_method}")
    monitor = SystemMonitor()
    
    # Initialize model
    print(f"\nTraining with {optimizer_type} optimizer and {weighting_method.upper()} weighting")
    print("\nModel Summary")
    ...
    # Calculate class weights - FIXED: Single calculation block
    class_weights_tensor = None
    raw_weights = None
    final_weights = None
    
    # Get class counts for all scenarios
    class_counts = np.zeros(num_classes)
    for _, label in train_dataset:
    ...
    if weighting_method != 'equal':
        ...
        # For equal weights, create dummy arrays for visualization
        raw_weights = np.ones(num_classes)
        final_weights = np.ones(num_classes)
        
        # Still visualize the distribution
        weight_info = visualize_class_weights(
            class_names, class_counts, raw_weights, final_weights, 
            logger, weighting_method
        )

    # Setup optimizer and criterion - FIXED: Use the correctly calculated weights
    optimizer, criterion = setup_optimizer(
        model, 
        optimizer_type=optimizer_type, 
        class_weights=class_weights_tensor,  # This is now properly defined
        device=device
    )
    ...
    if hasattr(criterion, 'weight') and criterion.weight is not None:
        ...
        print("\nNo weights in loss function (using equal weighting)")
