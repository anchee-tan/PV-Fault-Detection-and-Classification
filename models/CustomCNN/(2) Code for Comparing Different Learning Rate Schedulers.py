...
from torch.optim.lr_scheduler import StepLR, ExponentialLR, CosineAnnealingLR, LambdaLR, ReduceLROnPlateau
...
def get_class_weights(dataset, num_classes, method='ens', beta=0.9):
    """
...
def cyclical_lr(epoch, lr):
    """CyclicalLR: base_lr=0.001, max_lr=0.1, step_size=20"""
    base_lr = 0.001
    max_lr = 0.1
    step_size = 20
...
def setup_optimizer_scheduler(model, optimizer_type='AdamW', scheduler_type='plateau', class_weights=None, device='cuda', max_epochs=50, logger=None):
    ...
    Args:
        ...
        scheduler_type: Type of scheduler ('step', 'exponential', 'cosine', 'cylindrical', 'plateau', 'none')
    ...
    Returns:
        ...
        lr_schedule_data: Learning rate schedule data
    """
    ...
    schedulers = {
        ...
        'cosine': CosineAnnealingLR(
            ...
            eta_min=initial_lr * 0.01  # Minimum LR (1% of initial)
        ),
        'cylindrical': LambdaLR(
            optimizer,
            lr_lambda=lambda epoch: cyclical_lr(epoch, initial_lr) / initial_lr
        ),
        'plateau': ReduceLROnPlateau(
            ...
            threshold=0.0001,    # Threshold for measuring new optimum
    ...
    # Loss function with re-weighting techniques
    ...
    if scheduler_type == 'plateau':
        print("NOTE: ReduceLROnPlateau requires calling scheduler.step(metric) with validation loss/metric")
        print("Example: scheduler.step(val_loss) after each epoch")
...
def visualize_lr_schedule(scheduler_type, initial_lr, max_epochs, logger=None):
    ...
    elif scheduler_type == 'cylindrical':
        # Cylindrical scheduler using your cyclical_lr function
        lr_schedule = [
            cyclical_lr(epoch, initial_lr) 
            for epoch in range(max_epochs)
        ]
        title = 'Cylindrical Learning Rate Scheduler'
        
    elif scheduler_type == 'plateau':
        # ReduceLROnPlateau - simulated schedule for visualization
        # This is a rough approximation since actual behavior depends on loss values
        lr_schedule = []
        current_lr = initial_lr
        patience_counter = 0
        
        # Simulate plateau detection every 8-12 epochs with some randomness
        for epoch in range(max_epochs):
            if epoch > 0 and epoch % 8 == 0:  # Simulate plateau detection
                patience_counter += 1
                if patience_counter >= 5:  # patience = 5
                    current_lr *= 0.5  # factor = 0.5
                    current_lr = max(current_lr, 1e-8)  # min_lr = 1e-8
                    patience_counter = 0
            lr_schedule.append(current_lr)
        
        title = 'ReduceLROnPlateau Learning Rate Scheduler (Simulated)'
...
def training_model(train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset, num_classes, device, class_names, optimizer_type='AdamW', weighting_method='ens', scheduler_type='plateau', num_epochs=50):
    ...
    logger = PerformanceLogger(experiment_name=f"CustomCNN_{optimizer_type}_{weighting_method}_{scheduler_type}")
    ...
    for epoch in range(num_epochs):
        ...
        if scheduler is not None:
            if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                ...
                # All other schedulers step without arguments
                scheduler.step()
    ...
    if new_lr != current_lr:
        ...
        if val_loss < best_val_loss:
            ...
            # Save the best model
            best_model_path = f"C:/Users/tanan/Downloads/anchee_fyp2/customcnn_pv_fault_best.pth"
            torch.save({
                'epoch': best_epoch,
    ...
    plt.plot(range(1, num_epochs+1), learning_rates, 'g-')
