...
def create_mobilenetv2_model(num_classes):
    """
    Create MobileNetV2 for transfer learning with feature extraction only.
    All pre-trained layers are frozen, only the new classifier head is trained.

    Args:
        num_classes (int): The number of output classes for the classifier.
        
    Returns:
        torch.nn.Module: The configured MobileNetV2 model for transfer learning.
    """
    model = models.mobilenet_v2(pretrained=True)
    
    # Freeze ALL pre-trained layers (feature extraction only)
    for param in model.parameters():
        param.requires_grad = False
    
    print("MobileNetV2: Using as fixed feature extractor - all pre-trained layers frozen")
...
def report_model_params(model, model_name="MobileNetV2"):
...
def initialize_model(num_classes, device):
    ...
    model = create_mobilenetv2_model(num_classes, unfreeze_percentage=0.25)
...
def training_model(train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset, num_classes, device, class_names, optimizer_type='AdamW', weighting_method='ens', scheduler_type='cosine', num_epochs=35):
    ...
    logger = PerformanceLogger(experiment_name=f"MobileNetV2_{optimizer_type}_{weighting_method}_{scheduler_type}")
    monitor = SystemMonitor()
    
    # Initialize model
    print(f"\nTraining with {optimizer_type} optimizer, {weighting_method.upper()} weighting, and {scheduler_type} scheduler")
    print("\nModel Summary")
    model = create_mobilenetv2_model(num_classes).to(device)
    try:
    ...
    best_model_path = f"C:/Users/tanan/Downloads/anchee_fyp2/mobilenetv2_pv_fault_best2.pth"
    ...
    final_model_path = f"C:/Users/tanan/Downloads/anchee_fyp2/mobilenetv2_pv_fault_final.pth"
    torch.save({
    ...
    final_model = create_mobilenetv2_model(num_classes).to(device)
...
def main_flow():
    ...
    final_model_path = os.path.join(model_dir, "mobilenetv2_pv_fault_final.pth")
